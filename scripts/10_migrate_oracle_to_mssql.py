#!/usr/bin/env python3
"""Migrate a Oracle database to MSSQL using Bani connectors.

Usage:
    docker compose up -d oracle mssql
    export ORACLE_USER=bani_test ORACLE_PASS='bani_test'
    export MSSQL_USER=sa MSSQL_PASS='BaniTest123!'
    python scripts/20_migrate_oracle_to_mssql.py
"""
...
def main() -> int:
    ...
    source_schema = os.environ.get("ORACLE_SCHEMA", "BANI_TEST")
    target_schema = "dbo"

    try:
        print("Oracle -> MSSQL migration")
        print("=" * 50)

        print("Introspecting Oracle schema...")
