...
class OracleConnector(SourceConnector, SinkConnector):
    ...
    def create_table(self, table_def: TableDefinition) -> None:
        ...
        self._create_table_impl(
            table_def, shorten_names=False, use_identity=True,
        )
        ...
        with self._pool.acquire() as conn:
            ...
            try:
                for index in indexes:
                    ...
                    resolved_cols = [
                        self._resolve_name(col) for col in index.columns
                    ]
                    col_list = ", ".join(
                        f'"{col}"' for col in resolved_cols
                    )
                    ...
                    try:
                        ...
                        if any(
                            ...
                            _logger.warning(
                                ...
                                index.name, table_name, exc_str[:80],
                            )
                            continue
                        raise
            finally:
