...
def _ensure_registry_env_vars() -> None:
    """Populate ``_BANI_CONN_*`` env vars from ``connections.json``.

    BDL files generated by the MCP server use ``${env:_BANI_CONN_<key>_USER}``
    and ``${env:_BANI_CONN_<key>_PASS}`` references. The MCP server injects
    these into its own process via ``_ensure_env``, but the UI runs in a
    separate process. This function loads all connections and sets the env
    vars so the BDL credentials resolve correctly.

    It also pre-initialises Oracle thick mode if any Oracle connection has
    ``oracle_client_lib`` in its options — thick mode must be activated
    before any thin mode connection is created.
    """
    ...
    for _key, conn in ConnectionRegistry.load().items():
        ...
        # Pre-init Oracle thick mode if needed
