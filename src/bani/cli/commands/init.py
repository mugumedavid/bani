...
def init(
                ...
                json.dumps(
                    {
                        "command": "init",
                        "status": "error",
                        "error": "Missing --source flag (required in non-interactive mode)",
                    }
                )
                + "\n"
            )
                ...
                json.dumps(
                    {
                        "command": "init",
                        "status": "error",
                        "error": "Missing --target flag (required in non-interactive mode)",
                    }
                )
                + "\n"
            )
    ...
    if output_format == "json":
        ...
        console.print(
            "[dim]Edit the file to add table selections, type mappings, and hooks.[/dim]"
        )
...
<bani schemaVersion="1.0">
  <project name="{project_name}" description="Generated by bani init" author="bani" />
  <source connector="{source_connector}">
    <connection host="{source_host}" port="{source_port}" database="{source_database}"
                username="${{env:{source_user_env}}}" password="${{env:{source_pass_env}}}" />
  </source>
  <target connector="{target_connector}">
    <connection host="{target_host}" port="{target_port}" database="{target_database}"
                username="${{env:{target_user_env}}}" password="${{env:{target_pass_env}}}" />
  </target>
  <options>
